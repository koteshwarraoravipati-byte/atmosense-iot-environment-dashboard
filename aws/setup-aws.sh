...
{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["iot:Connect"],"Resource":["arn:aws:iot:$REGION:$ACCOUNT_ID:client/$THING_NAME"]},{"Effect":"Allow","Action":["iot:Publish"],"Resource":["arn:aws:iot:$REGION:$ACCOUNT_ID:topic/$TOPIC"]}]}
EOF
cat > "$OUT/dashboard-policy.json" <<EOF
{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["iot:Connect"],"Resource":["arn:aws:iot:$REGION:$ACCOUNT_ID:client/$DASHBOARD_THING_NAME"]},{"Effect":"Allow","Action":["iot:Subscribe"],"Resource":["arn:aws:iot:$REGION:$ACCOUNT_ID:topicfilter/$TOPIC"]},{"Effect":"Allow","Action":["iot:Receive"],"Resource":["arn:aws:iot:$REGION:$ACCOUNT_ID:topic/$TOPIC"]}]}
EOF
...
if [[ ! -s "$OUT/device-certificate.pem.crt" || ! -s "$OUT/device-private.pem.key" ]]; then create_cert device "$THING_NAME" "$DEVICE_POLICY_NAME"; fi
if [[ ! -s "$OUT/dashboard-certificate.pem.crt" || ! -s "$OUT/dashboard-private.pem.key" ]]; then create_cert dashboard "$DASHBOARD_THING_NAME" "$DASHBOARD_POLICY_NAME"; fi
...
chmod 600 "$OUT"/*-private.pem.key "$OUT"/*-certificate.pem.crt "$OUT/AmazonRootCA1.pem" 2>/dev/null || true
printf 'Provisioning complete. Endpoint: %s\nSecrets are in: %s\n' "$ENDPOINT" "$OUT"
