...
async function supabaseHistory(req,hours,deviceId='all',limit=2000,ascending=true){hours=safeHours(hours);let query=req.supabase.from('telemetry').select('id,device_id,temperature_c,humidity_percent,rssi_dbm,recorded_at').order('recorded_at',{ascending}).order('id',{ascending}).limit(limit);if(hours!=='all')query=query.gte('recorded_at',new Date(Date.now()-Number(hours)*3600000).toISOString());if(deviceId!=='all')query=query.eq('device_id',deviceId);const {data,error}=await query;if(error)throw error;return(data||[]).map(mapTelemetry)}
async function supabaseLatest(req){const history=await supabaseHistory(req,'all','all',1,false);return history[0]||null}

app.get('/api/latest',requireSession,async(req,res)=>{try{if(IS_SUPABASE){const latest=await supabaseLatest(req);return res.json({latest,recent:latest?[latest]:[],devices:await supabaseDevices(req),settings:await supabaseSettings(req)})}res.json({latest:recent.at(-1)||null,recent:recent.slice(-60),devices:devices.map(mapDevice),settings})}catch(error){res.status(500).json({error:'Unable to load dashboard data'})}});
