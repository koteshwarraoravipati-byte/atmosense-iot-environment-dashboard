...
async function supabaseLatest(req){const {data,error}=await req.supabase.from('telemetry').select('device_id,temperature_c,humidity_percent,rssi_dbm,recorded_at').order('recorded_at',{ascending:false}).limit(1);if(error)throw error;return(data||[]).map(mapTelemetry)[0]||null}
